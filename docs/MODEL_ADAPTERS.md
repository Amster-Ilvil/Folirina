...
## MangaLens / YOLO11 bubble segmentation
...
```json
{
  "bubbles": {
    "backend": "mangalens",
    "mangalens_model_path": "/absolute/path/to/best.pt",
    "mangalens_confidence": 0.35,
    "mangalens_imgsz": 1600
  }
}
```

可双击 `首次安装_MangaLens_气泡分割.command` 安装 Ultralytics 运行依赖。脚本**不会下载模型权重**。

模型返回的 instance mask 会直接用于 bubble geometry；safe area 在实例 mask 内向内收缩，因此不会退化成单纯 bbox 排字。
