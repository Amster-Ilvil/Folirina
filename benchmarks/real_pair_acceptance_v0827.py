"""Real-pair acceptance runner for v0.8.27 locked-raster transfer.
...
def main() -> int:
    ...
    ap.add_argument("--expect-applied", type=int, default=None)
    ns = ap.parse_args()
    out = Path(ns.output)
    cfg = PipelineConfig()
    ...
    applied_rows = [x for x in records if bool(x.get("applied"))]
    published_incomplete = [x for x in applied_rows if not bool(x.get("content_complete"))]
    ok = bool(summary.get("pass")) and not published_incomplete
    if ns.expect_records is not None:
        ok = ok and len(records) == ns.expect_records
    if ns.expect_applied is not None:
        ok = ok and len(applied_rows) == ns.expect_applied
