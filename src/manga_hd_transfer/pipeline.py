...
from .cache import (
    ...
    page_job_fingerprint, paired_diff_stage_signature, registration_stage_signature,
)
...
def _completion_filter_pairs_to_review_regions(source_bubbles, target_bubbles, review_boxes, config=None):
    ...
    def process_page(
        ...
        *,
        ...
        if (not direct_container_fast and mode in {"auto", "mask_replace", "hybrid"} and self.config.mask_replace.paired_diff_enabled
            ...
            paired_sig = paired_diff_stage_signature(pair, registration, self.config.mask_replace)
            paired_diff = stage_cache.load_paired_diff(paired_sig)
            if paired_diff is not None:
                cache_stats["paired_diff"] = "hit"
                ...
                try:
                    ...
                    stage_cache.save_paired_diff(paired_sig, paired_diff)
                    cache_stats["paired_diff"] = "miss"
                    use_paired_diff = bool(paired_diff.source_bubbles and paired_diff.target_bubbles)
                except Exception as exc:
                    cache_stats["paired_diff"] = "failed"
        ...
        if mode in {"auto", "direct_patch", "mask_replace", "hybrid"}:
            ...
            if (not direct_container_fast
                ...
                extra_src, extra_dst = _completion_filter_pairs_to_review_regions(extra_src, extra_dst, review_boxes, self.config.mask_replace)
                existing_boxes = [tuple(map(int, r.target_bbox)) for r in list(mask_transfer.records or [])
                                  if bool(getattr(r, "applied", False)) and getattr(r, "target_bbox", None)]
        ...
        if completion_display_source:
            known = {str(b.id) for b in source_bubbles}
            source_bubbles.extend([b for b in completion_display_source if str(b.id) not in known])
        if completion_display_target:
            known = {str(b.id) for b in target_bubbles}
            target_bubbles.extend([b for b in completion_display_target if str(b.id) not in known])
        ...
        # The primary/authority source equals the selected source on the common
        # path. Do not store the same lossless page twice. Secondary-source
        # arbitration still keeps the separate authority original.
        ...
        if bool(selected_secondary_source) or str(source_path_local) != str(authority_source_path):
            ...
            # Keep the historical artifact name for tools/tests, but share the
            # same inode so the common primary-source case consumes no extra disk.
        ...
        write_review_preview = bool(review_queue_for_preview) or bool(getattr(self.config.export, "save_review_preview_always", False))
        ...
        if self.config.export.save_masks:
            ...
            if bool(getattr(self.config.export, "save_component_masks", False)):
        ...
        if self.config.export.layer_bundle:
            ...
            # PSD needs an on-disk inpainted base; create it lazily only when the
            # user explicitly asked for the editable bundle.
            ...
            if not self.config.export.save_inpainted:
                inpainted_path.unlink(missing_ok=True)
        else:
        ...
        save_json(page_root / "qa.json", {"summary": qa_summary(qa), "issues": [x.to_dict() for x in qa]})
        save_json(page_root / "project.json", project.to_dict())
        # Remove leftovers from older full-diagnostic runs when the current
        # compact export settings no longer request them. Manual/review artifacts
        # are never touched by this conservative cleanup.
        if (not self.config.export.save_debug and not self.config.export.layer_bundle
                and not self.config.export.save_inpainted
                and not bool(getattr(self.config.export, "save_component_masks", False))):
