...
def run_cmd(
    ...
    mode: Optional[str] = typer.Option(None, "--mode", help="auto|direct_patch|mask_replace|hybrid|reletter"),
    ...
    if mode:
        if mode not in {"auto", "direct_patch", "reletter", "mask_replace", "hybrid"}:
            raise typer.BadParameter("--mode must be auto|direct_patch|mask_replace|hybrid|reletter")
