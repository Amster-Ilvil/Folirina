...
<html lang="zh-CN"><head><meta charset="utf-8"><title>Folirina Review</title>
<style>
...
</head><body><header><strong>Folirina · 出版级迁移 Review</strong><select id="pageSel"></select><button onclick="loadPage()">载入</button><span id="status"></span></header>
...
def validate_review_bind(host: str, allow_remote: bool = False) -> None:
    if _is_loopback_host(host):
        return
    if not allow_remote:
        raise ValueError(
            "Review 默认只允许绑定 localhost。若确实需要局域网访问，请显式传 --allow-remote；"
            "远程模式会启用一次性 URL token。"
        )
...
class ReviewServer:
    def __init__(self, output_dir: str | Path, config: PipelineConfig | None = None, *, token: str | None = None, require_url_token: bool = False) -> None:
        self.root = Path(output_dir).resolve()
        self.pages_root = self.root / "pages"
        self.config = config or PipelineConfig()
        self.token = str(token or secrets.token_urlsafe(24))
        self.require_url_token = bool(require_url_token)
        if not self.pages_root.exists():
            raise FileNotFoundError(f"No pages directory: {self.pages_root}")
        self.allowed_external_roots: list[Path] = []
        self.allowed_external_files: set[Path] = set()
        book_path = self.root / "book_project.json"
        if book_path.exists():
            try:
                book = load_json(book_path)
                for key in ("source_dir", "target_dir"):
                    raw = str(book.get(key) or "").strip() if isinstance(book, dict) else ""
                    if raw:
                        self.allowed_external_roots.append(Path(raw).resolve())
            except Exception:
                pass
        # Imported/legacy projects may not have book_project.json. Build an exact
        # allowlist from page project files so /asset cannot be turned into an
        # arbitrary local-file reader via query parameters.
        try:
            for page in self.pages_root.iterdir():
                project_path = page / "project.json"
                if not project_path.exists():
                    continue
                try:
                    project = normalize_project(load_json(project_path))
                    pair = as_dict(project.get("pair"))
                    for key in ("source_path", "target_path"):
                        raw = str(pair.get(key) or "").strip()
                        if raw:
                            self.allowed_external_files.add(Path(raw).resolve())
                except Exception:
                    continue
        except OSError:
            pass

    def external_asset_allowed(self, path: Path) -> bool:
        resolved = Path(path).resolve()
        if resolved in self.allowed_external_files:
            return True
        return any(_is_within(resolved, root) for root in self.allowed_external_roots)
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _cookie_token(self) -> str:
                ...
                for item in raw.split(";"):
                    ...
                    if sep and key == "mhd_review_token":
                        return value
                return ""

            def _authorized(self) -> bool:
                value = self._cookie_token()
                return bool(value) and secrets.compare_digest(value, server.token)

            def _require_authorized(self) -> bool:
                if self._authorized():
                    return True
                self._json({"error": "unauthorized review session"}, HTTPStatus.FORBIDDEN)
                return False

            def _valid_post_origin(self) -> bool:
                origin = str(self.headers.get("Origin", "") or "").strip()
                if not origin:
                    return True
                try:
                    return urlparse(origin).netloc == str(self.headers.get("Host", "") or "")
                except Exception:
                    return False
            ...
            def do_GET(self):
                ...
                try:
                    if u.path=="/":
                        supplied = str(q.get("token", [""])[0] or "")
                        if server.require_url_token and not secrets.compare_digest(supplied, server.token):
                            return self._json({"error": "remote review URL token required"}, HTTPStatus.FORBIDDEN)
                        data=_HTML.encode(); self.send_response(200); self.send_header("Content-Type","text/html; charset=utf-8")
                        self.send_header("Set-Cookie", f"mhd_review_token={server.token}; Path=/; HttpOnly; SameSite=Strict")
                    ...
                    if u.path=="/asset":
                        ...
                        if name=="source":
                            path=Path(project["pair"]["source_path"])
                            if not server.external_asset_allowed(path):
                                raise PermissionError("source asset is outside the review allowlist")
                            ...
                            elif mode=="hybrid" and (p/"hybrid_transfer_mask.png").exists():
                                path=(p/"manual_transfer_mask.png") if (p/"manual_transfer_mask.png").exists() else (p/"hybrid_transfer_mask.png")
                            elif mode in {"auto","mask_replace"} and (p/"mask_transfer_mask.png").exists():
            ...
            def do_POST(self):
                try:
                    ...
                    if not self._valid_post_origin():
                        return self._json({"error": "cross-origin review POST rejected"}, HTTPStatus.FORBIDDEN)
                    if "application/json" not in str(self.headers.get("Content-Type", "") or ""):
                        return self._json({"error": "application/json required"}, HTTPStatus.UNSUPPORTED_MEDIA_TYPE)
                    body=self._body(); p=server.page_dir(str(body.get("page","")))
                    if self.path=="/api/save-review":
                        with PageRunGuard(p, "review:http-save-review"):
                    ...
                    if self.path=="/api/save-mask":
                        ...
                        target=read_image(p/"target_original.png")
                        if mask.shape!=target.shape[:2]: raise ValueError("mask size mismatch")
                        ...
                        elif mode=="hybrid" and (p/"hybrid_transfer_mask.png").exists(): name="manual_transfer_mask.png"
                        elif mode in {"auto","mask_replace"} and (p/"mask_transfer_mask.png").exists(): name="manual_transfer_mask.png"
                        else: name="manual_clear_mask.png"
                        with PageRunGuard(p, "review:http-save-mask"):
                            write_image(p/name,mask)
                        return self._json({"ok":True,"mask":name})
...
def serve_review(
    output_dir: str | Path, host: str = "127.0.0.1", port: int = 8765,
    open_browser: bool = True, config: PipelineConfig | None = None,
    *, allow_remote: bool = False,
) -> None:
    validate_review_bind(host, allow_remote=allow_remote)
    remote = not _is_loopback_host(host)
    token = secrets.token_urlsafe(24)
    app = ReviewServer(output_dir, config, token=token, require_url_token=remote)
    httpd = ThreadingHTTPServer((host, port), app.handler())
    url = f"http://{host}:{port}/" + (f"?token={token}" if remote else "")
