...
def detect_unseeded_white_containers(
    ...
    *,
    ...
    for label in range(1, n):
        ...
        if compact < 2 and compact_area / max(1, cv2.countNonZero(inner)) < 0.004:
            continue
        polygon = mask_to_largest_polygon(component)
        if len(polygon) < 3:
            continue
...
def detect_mangalens_bubbles(
    ...
    The project deliberately does not download weights. ``mangalens_model_path``
    must point to a local YOLO segmentation checkpoint. This keeps the core
    package reproducible and lets the user choose the exact licensed model.
    """
    cfg = config or BubbleConfig()
    if not cfg.mangalens_model_path:
        raise ValueError("Bubble backend 'mangalens' requires bubbles.mangalens_model_path")
    try:
        from ultralytics import YOLO
    except Exception as e:
        raise RuntimeError("MangaLens backend requires the optional 'ultralytics' package") from e

    model_path = str(Path(cfg.mangalens_model_path).expanduser().resolve())
