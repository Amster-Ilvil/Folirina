    ...
    return PageProject(
        ...
        meta={
            ...
            "reletter": {
                ...
                "used": bool(mode == "reletter" and lettering),
                ...
                "target_driven_region_diagnostics": dict(target_driven_reletter_diagnostics or {}) if mode == "reletter" else {},
                "textbox_safe_region_used": int(constrained_layout_units) if mode == "reletter" else 0,
                "target_layout_hint_units": int(target_layout_hint_units) if mode == "reletter" else 0,
                "ocr_source_route": str(cache_stats.get("ocr_source", "")) if mode == "reletter" else "",
                "ocr_target_route": str(cache_stats.get("ocr_target", "")) if mode == "reletter" else "",
                "configured_font_path": str(config.lettering.font_path or "") if mode == "reletter" else "",
                "layout_mode": str(getattr(config.lettering, "layout_mode", "smart_scaling")) if mode == "reletter" else "",
                "resolved_font_path": str(next((x.font_path for x in lettering if getattr(x, "font_path", "")), "")) if mode == "reletter" else "",
                "successful_regions": int(sum(1 for x in lettering if bool(getattr(x, "success", False)))) if mode == "reletter" else 0,
                "failed_regions": int(sum(1 for x in lettering if not bool(getattr(x, "success", False)))) if mode == "reletter" else 0,
                "failed_reasons": [str(getattr(x, "reason", "") or "unknown") for x in lettering if not bool(getattr(x, "success", False))] if mode == "reletter" else [],
                "editable_regions": list(composition.reletter_editable_regions) if mode == "reletter" else [],
            },
            ...
            "mask_replace": {
                "requested": bool(mode == "mask_replace"),
                "used": bool(mask_transfer is not None and not direct_container_fast and mode in {"auto", "mask_replace"}),
                "strict_no_ocr_reletter": bool(mode == "mask_replace"),
                "mode_contract_no_text_renderer": bool(mode == "mask_replace"),
                "applied_count": mask_transfer.applied_count if (mask_transfer is not None and not direct_container_fast and mode in {"auto", "mask_replace"}) else 0,
                "records": transfer_records_to_dict(active_records) if (not direct_container_fast and mode in {"auto", "mask_replace"}) else [],
                "bubble_matches": [m.to_dict() for m in active_matches] if (not direct_container_fast and mode in {"auto", "mask_replace"}) else [],
                ...
                "manual_reletter_required": mask_manual_reletter if mode in {"auto", "mask_replace"} else [],
                "review_regions": active_review_regions if (not direct_container_fast and mode in {"auto", "mask_replace"}) else [],
            },
            "hybrid": {
                ...
                "mask_records": transfer_records_to_dict(active_records) if mode == "hybrid" else [],
                "bubble_matches": [m.to_dict() for m in active_matches] if mode == "hybrid" else [],
                "manual_reletter_required": mask_manual_reletter if mode == "hybrid" else [],
