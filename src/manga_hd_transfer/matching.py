...
def match_units_by_paired_ids(
    source_units: list[TextUnit],
    target_units: list[TextUnit],
    registration: RegistrationResult,
    config: MatchingConfig | None = None,
) -> MatchResult:
    """Directly bind units using paired-diff bubble identity.

    Reletter mode already knows which SOURCE and TARGET containers belong
    together via paired-diff extraction. Re-running Hungarian assignment on top
    of those explicit identities can still drift when a phone photo bends one
    balloon or when reading-order sorting differs slightly between the two sides.
    This helper keeps the paired region IDs authoritative while leaving other
    modes on the normal geometric matcher.
    """
    cfg = config or MatchingConfig()
    if not source_units or not target_units:
        return MatchResult(
            [], [u.id for u in source_units], [u.id for u in target_units], diagnostics={
                "route": "paired_id_binding",
                "registration_confidence": float(registration.confidence),
            }
        )

    target_index_by_region = {str((u.meta or {}).get("reletter_region_id") or ""): j for j, u in enumerate(target_units) if str((u.meta or {}).get("reletter_region_id") or "")}
    target_by_region = {str((u.meta or {}).get("reletter_region_id") or ""): u for u in target_units if str((u.meta or {}).get("reletter_region_id") or "")}
    source_index_by_region = {str((u.meta or {}).get("reletter_region_id") or ""): i for i, u in enumerate(source_units) if str((u.meta or {}).get("reletter_region_id") or "")}
    source_by_region = {str((u.meta or {}).get("reletter_region_id") or ""): u for u in source_units if str((u.meta or {}).get("reletter_region_id") or "")}
    target_index_by_bubble = {str(u.bubble_id): j for j, u in enumerate(target_units) if getattr(u, "bubble_id", None)}
    target_by_bubble = {str(u.bubble_id): u for u in target_units if getattr(u, "bubble_id", None)}
    source_index_by_bubble = {str(u.bubble_id): i for i, u in enumerate(source_units) if getattr(u, "bubble_id", None)}
    source_by_bubble = {str(u.bubble_id): u for u in source_units if getattr(u, "bubble_id", None)}
    ...
    def _direct_confidence() -> float:
        base = 0.86 + 0.12 * float(np.clip(registration.confidence, 0.0, 1.0))
        return float(np.clip(base, cfg.review_confidence, 0.995))

    direct_conf = _direct_confidence()
    for i, src in enumerate(source_units):
        meta = getattr(src, "meta", {}) or {}
        target_region_id = str(meta.get("paired_target_region_id") or meta.get("reletter_region_id") or "")
        dst = target_by_region.get(target_region_id) if target_region_id else None
        j = target_index_by_region.get(target_region_id) if target_region_id else None
        reason_id = f"paired_target_region_id={target_region_id}" if target_region_id else ""
        tid = ""
        if dst is None:
            tid = str(meta.get("paired_target_id") or "")
            if not tid and i < len(target_units) and len(source_units) == len(target_units):
                # Conservative fallback for synthetic paired-region routes whose units
                # were created in already-paired order. Never used as a cross-mode rule.
                cand = target_units[i]
                if getattr(src, "bubble_id", None) and getattr(cand, "bubble_id", None):
                    tid = str(cand.bubble_id)
            dst = target_by_bubble.get(tid)
            j = target_index_by_bubble.get(tid)
            reason_id = f"paired_target_id={tid}"
        if dst is None or j is None or j in used_t:
            continue
        matches.append(UnitMatch(
            ...
            relation="one_to_one",
            reasons=[
                f"route=paired_id_binding",
                reason_id,
                f"registration_confidence={float(registration.confidence):.3f}",
            ],
        ))
        diagnostics_rows.append({"source_unit_id": src.id, "target_unit_id": dst.id, "paired_target_region_id": target_region_id, "paired_target_id": tid})
        used_s.add(i)
        used_t.add(j)

    # Recover target→source links that may exist only on the target side.
    for j, dst in enumerate(target_units):
        if j in used_t:
            continue
        sid = str((getattr(dst, "meta", {}) or {}).get("paired_source_id") or "")
        if not sid:
            continue
        src = source_by_bubble.get(sid)
        i = source_index_by_bubble.get(sid)
        if src is None or i is None or i in used_s:
            continue
        matches.append(UnitMatch(
            source_unit_id=src.id,
            target_unit_id=dst.id,
            ...
            reasons=[
                f"route=paired_id_binding",
                f"paired_source_id={sid}",
                f"registration_confidence={float(registration.confidence):.3f}",
            ],
        ))
        diagnostics_rows.append({"source_unit_id": src.id, "target_unit_id": dst.id, "paired_source_id": sid})
        used_s.add(i)
        used_t.add(j)

    unmatched_s = [source_units[i].id for i in range(len(source_units)) if i not in used_s]
    unmatched_t = [target_units[j].id for j in range(len(target_units)) if j not in used_t]
    diagnostics = {
        "route": "paired_id_binding",
        "registration_confidence": float(registration.confidence),
        "pairs": diagnostics_rows,
        "top_candidates": {},
        "rejected_over_max_cost": [],
        "ambiguities": [],
        "force_actions": ["force_match", "skip_unit", "force_direct_patch", "force_mask_replace"],
    }
    return MatchResult(matches, unmatched_s, unmatched_t, [], diagnostics)
