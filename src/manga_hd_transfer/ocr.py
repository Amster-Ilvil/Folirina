...
class PaddleOCRBackend(OCRBackend):
    def __init__(self, lang: str, config: OCRConfig | None = None, device: str | None = None) -> None:
        ...
        det_dir = self.config.paddle_text_detection_model_dir or (str(local_det) if local_det else None)
        rec_dir = self.config.paddle_text_recognition_model_dir or (str(local_rec) if local_rec else None)
        if det_dir:
