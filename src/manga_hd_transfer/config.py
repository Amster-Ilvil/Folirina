...
class TransferModeConfig(BaseModel):
    # Direct Patch and Mask Replace are deliberately separate contracts.
    # auto: Source-Direct first, then mask/hybrid fallbacks as needed.
    mode: str = "auto"  # auto|direct_patch|mask_replace|aligned_overlay_reveal|hybrid|reletter
...
class MaskReplaceConfig(BaseModel):
    ...
    # optional RT-DETR/MangaLens are lazy and never run on TARGET in precise mode.
    source_direct_detector_chain: list[str] = Field(default_factory=lambda: ["pseudo_text_barrier", "sidecar", "rtdetr_v2", "sam2", "mangalens"])
...
class DirectPatchConfig(MaskReplaceConfig):
...
class AlignedOverlayRevealConfig(BaseModel):
    ...
    min_registration_confidence: float = 0.80
...
class OCRConfig(BaseModel):
    backend: str = "paddle"  # paddle|apple|apple_live_text|apple_shortcut|sidecar|none
    source_backend: str | None = None
    target_backend: str | None = None
    source_lang: str = "ch"
    target_lang: str = "japan"
    ocr_version: str = "PP-OCRv5"
    # Optional explicit PP-OCRv5 local model directories.  When set, PaddleOCR
    # does not need to contact a model hub at runtime.
...
class BubbleConfig(BaseModel):
    backend: str = "seeded_white"  # seeded_white|mangalens|rtdetr_v2|sam2|sidecar|none
...
class InpaintingConfig(BaseModel):
    backend: str = "auto"  # auto|solid|threshold_clear|opencv|lama
