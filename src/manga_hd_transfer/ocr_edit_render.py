...
from .ocr_review_lettering import fit_text, composite_text
from .ocr_review_layout_region import recover_layout_region, infer_text_orientation
from .manual_effect import clean_manual_target_text
from .text_only_transfer import clear_broad_neutral_paper_components
...
def _mode_text_layer_mask(page_dir: Path, mode: str, shape: tuple[int, int]) -> np.ndarray:
    """Return the mode-owned automatic OCR text alpha, when one exists."""
    names: tuple[str, ...]
    if mode == "reletter":
        names = ("reletter_text_layer.png", "text_layer.png")
    elif mode == "hybrid":
        names = ("hybrid_text_layer.png", "text_layer.png")
    else:
        names = ()
    for name in names:
