...
def detect_source_rtdetr_v2(source: np.ndarray, mask_cfg: MaskReplaceConfig, bubble_cfg: BubbleConfig, existing=None, **kwargs) -> list[BubbleInstance]:
    ...
    if resolved is not None:
        bubble_cfg.rtdetr_model_path=str(resolved)
    ...
    revision = _local_model_revision(source_ref) if model_path else (source_ref, 'remote')
    key=(source_ref,device,imgsz,revision)
    ...
    if runtime is None:
        try:
            ...
            for old_key in [k for k in _RTDETR_RUNTIME if k[:3] == key[:3] and k != key]:
                _RTDETR_RUNTIME.pop(old_key, None)
...
def detect_source_sam2(source: np.ndarray, mask_cfg: MaskReplaceConfig, bubble_cfg: BubbleConfig, existing=None, **kwargs) -> list[BubbleInstance]:
    ...
    if resolved is not None:
        bubble_cfg.sam2_checkpoint=str(resolved)
    ...
    if checkpoint:
        ...
        revision = _local_model_revision(ckpt)
        key=(ckpt,config_file,device,revision)
        ...
        if predictor is None:
            try:
                ...
                for old_key in [k for k in _SAM2_RUNTIME if len(k) >= 3 and k[:3] == key[:3] and k != key]:
                    _SAM2_RUNTIME.pop(old_key, None)
