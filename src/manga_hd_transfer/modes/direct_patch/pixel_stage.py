...
def _resolve_active_pixel_mode_modules(*, mode: str, direct_container_fast: bool):
    ...
    return "direct_patch", transfer_ops, global_registered_raster
...
def run_pixel_transfer_stage(
    *, config, pair_check, registration, mode, mode_contract, source, target,
    ...
    if str(mode or "").strip().lower() not in {"direct_patch", "auto"}:
        raise RuntimeError("direct_patch pixel stage cannot execute mode=" + str(mode))
