...
def _active_mask_config(config: Any, mode: str):
    return config.hybrid.mask


def _resolve_hybrid_fallback_policy(mode: str):
    from .fallback_policy import HYBRID_SOURCE_INTEGRITY_BLOCK_REASONS, hybrid_mask_result_complete
    return frozenset(HYBRID_SOURCE_INTEGRITY_BLOCK_REASONS), hybrid_mask_result_complete
...
def _resolve_active_pixel_mode_modules(*, mode: str, direct_container_fast: bool):
    ...
    return "hybrid", transfer_ops, global_registered_raster
...
def _hybrid_mask_claimed_target_boxes(transfer_result: Any | None) -> list[tuple[int, int, int, int]]:
    """Return TARGET boxes already claimed by Hybrid's private Mask stage.

    For the product contract, a detected Mask candidate owns the region even if
    it is later restored for review. Automatic OCR must not silently replace a
    region merely because the mask audit was conservative. Manual OCR boxes are
    a separate explicit user action and bypass this automatic gate.
    """
...
def _hybrid_filter_auto_ocr_matches(
    rows: list[Any],
    target_units: list[Any],
    transfer_result: Any | None,
    cfg: Any,
    *,
    mask_candidate_bubbles: list[BubbleInstance] | None = None,
) -> tuple[list[Any], dict[str, int | bool]]:
    enabled = bool(getattr(cfg, "hybrid_auto_ocr_uncovered_only", True))
    block_any_mask = bool(getattr(cfg, "hybrid_auto_ocr_block_if_any_mask_candidate", True))
    if not enabled or not block_any_mask or not rows:
        return list(rows), {
            "enabled": enabled, "block_any_mask_candidate": block_any_mask,
            "claimed_boxes": 0, "input_matches": len(rows), "kept_matches": len(rows),
        }
    boxes = _hybrid_mask_claimed_target_boxes(transfer_result)
    # Product contract is candidate-owned, not record-owned. A Mask proposal can
    # be rejected before a transfer record is emitted; automatic OCR must still
    # stay out of that region. Manual OCR boxes bypass this helper entirely.
    for bubble in list(mask_candidate_bubbles or []):
        try:
            box = tuple(int(v) for v in bubble.bbox)
        except Exception:
            continue
    ...
    by_id = {str(getattr(u, "id", "")): u for u in list(target_units or [])}
    kept: list[Any] = []
    for match in rows:
        unit = by_id.get(str(getattr(match, "target_unit_id", "")))
        if unit is None:
            kept.append(match)
            continue
        cx, cy = unit.centroid
        covered = any(x0 <= cx <= x1 and y0 <= cy <= y1 for x0, y0, x1, y1 in boxes)
        if not covered:
            kept.append(match)
    return kept, {
        "enabled": enabled, "block_any_mask_candidate": block_any_mask,
        "claimed_boxes": len(boxes), "input_matches": len(rows), "kept_matches": len(kept),
    }
...
def run_pixel_transfer_stage(
    *, config, pair_check, registration, mode, mode_contract, source, target,
    ...
    if str(mode or "").strip().lower() not in {"hybrid", "auto"}:
        raise RuntimeError("hybrid pixel stage cannot execute mode=" + str(mode))
    ...
    if mode_contract.direct or mode_contract.mask_replace:
        ...
        if (not direct_container_fast
            ...
            auto_ocr_mask_candidates = list(mask_target_bubbles or [])
            if supplement is not None:
                auto_ocr_mask_candidates.extend(list(getattr(supplement, "target_bubbles", []) or []))
            auto_ocr_mask_candidates.extend(list(completion_display_target or []))
            ocr_guided_matches, ocr_gate_diag = _hybrid_filter_auto_ocr_matches(
                list(matches), list(target_units), mask_transfer, mask_cfg,
                mask_candidate_bubbles=auto_ocr_mask_candidates,
            )
            if cache_stats is not None:
                cache_stats["hybrid_auto_ocr_guided_gate"] = str(ocr_gate_diag)
            ocr_extra = transfer_ocr_guided_text_units(
                aligned_for_ocr, mask_transfer.image, source_units, target_units,
                ocr_guided_matches, registration, mask_cfg,
                exclude_mask=ocr_exclude_mask,
            ) if ocr_guided_matches else None
            if ocr_extra is not None and ocr_extra.records:
        ...
        if mode == "hybrid":
            # v2.3.36 product contract: Hybrid is strictly Mask-first. Any region
            # that already has a Mask-stage candidate is owned by that mask result
            # (SAFE, REVIEW, or restored-for-review) and is never automatically
            # re-OCRed. Automatic OCR/reletter is only for truly uncovered regions.
            # The manual OCR box editor remains an explicit user override.
            ...
            if supplement is not None:
                auto_ocr_mask_candidates.extend(list(getattr(supplement, "target_bubbles", []) or []))
            auto_ocr_mask_candidates.extend(list(completion_display_target or []))
            fallback_matches, auto_ocr_gate_diag = _hybrid_filter_auto_ocr_matches(
                fallback_seed, list(target_units), mask_transfer, mask_cfg,
                mask_candidate_bubbles=auto_ocr_mask_candidates,
            )
            if cache_stats is not None:
                cache_stats["hybrid_mask_complete_records"] = str(len(complete_dst_ids))
                cache_stats["hybrid_auto_ocr_gate"] = str(auto_ocr_gate_diag)
