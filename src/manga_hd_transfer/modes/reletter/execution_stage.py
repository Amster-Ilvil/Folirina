...
def _resolve_mode_runtime_primitives(mode: str, *, direct_container_fast: bool):
    ...
    return "reletter", masking_ops, inpainting_ops, lettering_ops, ocr_cleanup
...
def run_transfer_execution_stage(
    *, config, pair, registration, pair_check, mode, mode_contract, source, target,
    ...
    if str(mode or "").strip().lower() not in {"reletter"}:
        raise RuntimeError("reletter execution stage cannot execute mode=" + str(mode))
    ...
    if mode in {"auto", "mask_replace"} and mask_transfer is None:
        ...
        for match in fallback_matches:
            ...
            lcfg = config.reletter.lettering.model_copy(deep=True)
            ...
            if result.success and result.text_mask is not None:
                rendered = composite_text(rendered, result, config.reletter.lettering)
