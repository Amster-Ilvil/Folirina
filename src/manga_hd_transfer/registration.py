...
def _deep_registration_runtime_ready() -> bool:
    try:
        from .deep_registration_runtime import runtime_status
        return bool(runtime_status().ready)
    except Exception:
        return False
...
def _deep_registration_matches(kind: str, source: np.ndarray, target: np.ndarray, cfg: RegistrationConfig) -> FeatureMatches:
    """Run Torch-based page matching outside the Qt/GUI Python process.

    The main app may run on Python 3.14 where an otherwise discoverable Torch
    build can still fail during distributed/RPC initialisation.  LightGlue and
    LoFTR therefore live in the persistent isolated deep-registration worker.
    """
    try:
        from .deep_registration_runtime import run_deep_registration
        sp, tp, method, diagnostics = run_deep_registration(
            kind, source, target,
            feature=cfg.feature,
            max_features=int(cfg.max_features),
            deep_max_side=int(cfg.deep_max_side),
            device=str(cfg.device),
        )
    except Exception as exc:
        label = "LightGlue" if kind == "lightglue" else "LoFTR"
        raise RuntimeError(
            f"{label} 独立配准运行时不可用。请在“模型下载与接入状态”中点击 {label} 的“安装依赖”，"
            "不要把 Torch 强装进 GUI 的 Python 3.14。\n" + str(exc)
        ) from exc
    diagnostics = dict(diagnostics or {})
    diagnostics.setdefault("isolated_runtime", True)
    return FeatureMatches(sp, tp, method, diagnostics)


def _lightglue_matches(source: np.ndarray, target: np.ndarray, cfg: RegistrationConfig) -> FeatureMatches:
    return _deep_registration_matches("lightglue", source, target, cfg)


def _loftr_matches(source: np.ndarray, target: np.ndarray, cfg: RegistrationConfig) -> FeatureMatches:
    return _deep_registration_matches("loftr", source, target, cfg)


def _lightglue_matches_resilient(source: np.ndarray, target: np.ndarray, cfg: RegistrationConfig) -> FeatureMatches:
    # Device fallback is handled inside the isolated worker, where a failed MPS
    # kernel cannot poison the GUI's Torch module state.
...
def register_images(source: np.ndarray, target: np.ndarray, config: RegistrationConfig | None = None) -> RegistrationResult:
    ...
    if backend == "auto":
        ...
        if _deep_registration_runtime_ready() and _deep_weights_ready("lightglue", cfg):
            try:
        ...
        if _deep_registration_runtime_ready() and _deep_weights_ready("loftr", cfg):
            try:
