...
def page_id_for_pair(pair: PagePair) -> str:
    for reason in list(getattr(pair, "reasons", []) or []):
        if isinstance(reason, str) and reason.startswith("restored_page_id:"):
            restored = reason.split(":", 1)[1].strip()
            if restored:
                return restored
...
def resolve_page_workspace(
    ...
    explicit_page_root: str | Path | None = None,
) -> PageWorkspace:
    ...
    root: Path | None = None
    if explicit_page_root:
        try:
            candidate = Path(explicit_page_root)
            if candidate.exists() and candidate.is_dir():
                root = candidate
        except (TypeError, ValueError, OSError):
            root = None
    if root is None and output_dir:
        try:
    ...
    transparent_meta = normalize_route_meta(meta.get("transparent_bubble_reveal"))
    direct_used = bool(direct_meta.get("used"))
    transparent_route = bool(
        transparent_meta.get("used")
        or str(meta.get("transfer_mode", "")) == "transparent_bubble_reveal"
        or str(meta.get("transfer_planner", {}).get("strategy", "")) == "transparent_bubble_reveal"
    )
    aligned_route = bool(
    ...
    if transparent_route:
        candidates = [artifacts.get("clear_mask"), artifacts.get("target_clear_mask")]
        if root is not None:
            candidates[:0] = [root / "clear_mask.png", root / "target_clear_mask.png"]
