...
def _component_from_seed(
    ...
    *,
    ...
    """Return the white connected component nearest one seed.

    A page probes many bubble candidates against the same SOURCE/TARGET canvas.
    Thresholding, closing, and connected-component labelling depend only on the
    image + threshold, not on the seed. Reuse those deterministic intermediates
    within a single paired-diff request; the returned component remains exactly
    the same as the uncached implementation.
    """
    cache = cache if cache is not None else {}
    key = ("white_components", id(image), image.shape, image.strides, int(threshold))
    row = cache.get(key)
    if row is None:
...
def _extract_raw(
    ...
    for label in range(1, n):
        ...
        tm = _component_from_seed(target, cx, cy, cfg.paired_diff_white_threshold, cfg.paired_diff_search_radius, cache=component_cache)
        if tm is None or cv2.countNonZero(tm) < cfg.paired_diff_min_component_area:
            continue
        sx, sy = _transform_xy(cx, cy, Hinv)
        sm = _component_from_seed(source, sx, sy, cfg.paired_diff_white_threshold, cfg.paired_diff_search_radius, cache=component_cache)
...
def _photo_pair_fallback(
    ...
    for cand in target_candidates[: cfg.photo_pair_max_candidates]:
        ...
        for source_threshold in thresholds:
            sm = _component_from_seed(source, sx, sy, int(source_threshold), source_radius, cache=component_cache)
            if sm is None or cv2.countNonZero(sm) < cfg.paired_diff_min_component_area:
                continue
            warped_sm = _warp_mask(sm, H, target_shape)
