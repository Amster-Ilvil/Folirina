...
from .mask_transfer import BubblePatchMatch, MaskTransferRecord, MaskTransferResult
from .models import BubbleInstance, RegistrationResult
from .plugins import REGISTRY as PROVIDER_REGISTRY
from .text_only_transfer import transfer_text_only
...
def _constrain_direct_write_to_target_border(
    ...
    line_protected = boundary_band & strong_target_line
    ...
    # Everything removed from the original Direct mask plus detected target-line
    # pixels is protected and may be restored byte-for-byte after compositing.
    protected = ((use > 0) & ~safe) | line_protected
...
def build_source_direct_container_plan(
    ...
    for contour, is_source_hint, hint_backend in contour_entries:
        ...
        if aggressive and not is_source_hint and compact < 2 and compact_ratio < 0.006:
        ...
        if (
            ...
            rejected_by_artwork += 1
            continue
        if (compact < 1 or compact_ratio < 0.003) and not spiky:
            continue
        if (
            _publication_safety_enabled(cfg)
            and not spiky
        ...
        rec.spill_ratio = 0.0
        rec.local_dx = float(best_dx); rec.local_dy = float(best_dy)
        ...
        rec.content_check = "checked_text_only_target_background_preserved"
        rec.source_ink_coverage = 1.0
        rec.target_residual_ratio = 0.0
        rec.content_complete = True
        rec.clarity_mode = "direct-text-only-target-background"
    ...
    diagnostics = {
        ...
        "final_raster_transform": "local_similarity_only",
