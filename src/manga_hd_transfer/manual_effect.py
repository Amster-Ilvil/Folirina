...
def _augment_colored_manual_text_masks(
    ...
    # Once a saturated container is confidently identified, its safe interior
    # is stronger evidence than the cross-rendition difference mask: the latter
    # often treats the entire red/yellow fill or burst outline as "changed".
    # Use compact text inside the colour interior as the authoritative mask.
    src_out=src_extra if cv2.countNonZero(src_extra)>=8 else source_mask
    tgt_out=merged_tgt if cv2.countNonZero(merged_tgt)>=8 else target_clear_mask
    cdiag.update({
        "source_extra_pixels":int(cv2.countNonZero(src_extra)),
        "target_extra_pixels":int(cv2.countNonZero(tgt_extra)),
        "baseline_source_pixels":baseline_src,
        "baseline_target_pixels":baseline_tgt,
        "merged_target_pixels":int(cv2.countNonZero(merged_tgt)),
        "colored_target_corridor_radius":int(corridor_radius),
        "authoritative_colored_text_masks":bool(cv2.countNonZero(src_extra)>=8 and cv2.countNonZero(merged_tgt)>=8),
...
def clean_manual_target_text(
    ...
    *,
    ...
    Flat saturated containers use their own TARGET fill median.  Complex areas
    fall back to per-component local Telea and copy back only masked pixels.
    This mirrors the high-level behaviour of mature manga translators while
    preserving this project's TARGET-background-authority contract.
    """
    ...
    if safe is not None and cv2.countNonZero(safe)>0:
        use=(safe>0)
        exclude=cv2.dilate(mask,cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7)),iterations=1)>0
        ...
        if samples.shape[0]>=40:
            ...
            # Robust spread is tolerant of JPEG grain but rejects gradients/art.
            if float(np.max(mad))<=18.0:
                sel=(mask>0)&use
                out[sel]=np.clip(np.round(med),0,255).astype(np.uint8)
                return out,{"mode":"target_flat_color","pixels":int(np.count_nonzero(sel)),"median_bgr":[float(x) for x in med],"mad_max":float(np.max(mad)),"container":cdiag}
...
def build_manual_effect_masks(
    ...
    if mode in {"full_patch", "white_bubble_text"}:
        ...
        if mode in {"reveal_text", "effect_text"}:
            ...
            if mode == "effect_text" and bool(color_diag.get("authoritative_colored_text_masks")):
