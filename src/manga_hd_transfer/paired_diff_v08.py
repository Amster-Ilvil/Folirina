...
def _barrier_component_from_seed(
    ...
    This is considerably more robust on photographed pages than thresholding for
    white pixels: page curl changes white levels but the black balloon outline is
    still a strong topological barrier.
    """
...
def _source_component_for_target(
    ...
    for thr in (base - 20, base - 10, base, base + 10, base + 20):
        for dp in sorted({1, max(1, int(cfg.paired_diff_barrier_dilate_px)), 2}):
            sm = _barrier_component_from_seed(source, sx, sy, cfg, int(np.clip(thr, 130, 225)), dp)
...
def _find_enclosed_candidates(
    ...
    for label in range(1, n):
        ...
        tm = _barrier_component_from_seed(target, cx, cy, cfg)
        if tm is None:
...
def extract_paired_diff_bubbles(
    ...
    H = transform_to_homography(registration.matrix)
    enclosed = _find_enclosed_candidates(target, raw_change, regions, cfg)
    ...
    for tm, metrics, box in enclosed:
        ...
        sm, mask_iou = _source_component_for_target(source, tm, (cx, cy), H, cfg)
