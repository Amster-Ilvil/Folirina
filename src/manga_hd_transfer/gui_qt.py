...
from .model_downloads import download_builtin_model, apply_config_updates, model_home, model_local_paths, import_builtin_model, diagnose_download_network, apply_download_network_settings
from .dependency_install import install_paddle_ocr_dependencies
from .io_utils import load_json, save_json, write_image
from .review_apply import apply_review_page, generate_remove_text_preview, apply_target_layer_erase_review, reset_target_layer_erase_review
...
QLabel#appTitle {{ font-size:17px; font-weight:750; letter-spacing:.1px; }}
...
class DependencyInstallWorker(QThread):
    ...
    def run(self):
        try:
            ...
            result = install_paddle_ocr_dependencies(lambda msg: self.progress.emit(str(msg)))
            self.done.emit(result)
        except Exception as exc:
            import traceback
            self.failed.emit(f"{exc}\n\n{traceback.format_exc()}")
...
class MaskEditorDialog(QDialog):
    def __init__(self, image_path: str | Path, initial_mask: Any, parent=None, *, title: str = "清除蒙版编辑器", hint_text: str | None = None, save_label: str = "保存蒙版", preview_fn=None):
        ...
        self.focus_button=QPushButton("聚焦蒙版"); self.fit_button = QPushButton("查看整页"); self.clear_button = QPushButton("清空蒙版"); self.save_button = QPushButton(save_label); self.save_button.setObjectName("primary"); self.cancel_button = QPushButton("取消")
        action_row.addWidget(self.focus_button); action_row.addWidget(self.fit_button); action_row.addWidget(self.clear_button); action_row.addStretch(1); action_row.addWidget(self.save_button); action_row.addWidget(self.cancel_button); root.addLayout(action_row)
        self.slider.valueChanged.connect(self._brush); self.fit_button.clicked.connect(self.view.fit_to_window); self.focus_button.clicked.connect(self.view.fit_to_mask)
        ...
        QTimer.singleShot(0, self.view.fit_to_mask if bool(self.view.mask.any()) else self.view.fit_to_window)

    def _brush(self, value: int):
        self.view.brush_size = int(value); self.size_label.setText(f"{int(value)} px")

    def _clear(self):
        self.view.mask[:] = 0; self.view._refresh_overlay(); self._refresh_live_preview()
...
class ProjectPage(QWidget):
    ...
    def __init__(self, window: "StudioWindow"):
        ...
        self.restore_btn.setToolTip("读取命令行/Codex 已生成的输出目录，恢复 SOURCE/TARGET、final、中间产物和人工复核状态，然后继续在 GUI 补漏。不会重新处理或覆盖已有结果。")
        ...
        self.thumb_list = StableThumbnailList(); self.thumb_list.setViewMode(QListWidget.ViewMode.IconMode)
        ...
        mode.layout.addWidget(self.show_experimental); mode.layout.addWidget(self.experimental_warning); mode.layout.addWidget(self.experimental_status); mode.layout.addWidget(self.experimental_result_status)
    ...
    def _update_experimental_status(self):
        enabled = bool(self.show_experimental.isChecked())
        current = str(self.mode.currentData() or "")
        if not enabled:
            self.experimental_status.setVisible(False)
            self.mode.setToolTip("当前迁移方式")
            return
        if current == 'aligned_overlay_reveal':
            self.experimental_status.setVisible(True)
            self.experimental_status.setText('实验状态：已启用且当前下拉框就是“实验 · 整页对齐挖洞显中文”。重新处理当前页/整册后才会真正生效。')
            self.experimental_status.setStyleSheet(f'color:{GREEN}; background:{GREEN_SOFT}; border:1px solid #BFE3D5; border-radius:8px; padding:6px 8px;')
            self.experimental_status.setToolTip('绿色 = 下一次处理会跑整页对齐挖洞显中文实验路线')
            self.mode.setToolTip('实验模式已真正启用：当前下拉框就是“实验 · 整页对齐挖洞显中文”')
        else:
            self.experimental_status.setVisible(True)
            self.experimental_status.setText('实验状态：你已勾选“显示并启用实验模式”，但当前下拉框不是实验项，所以本次不会跑挖洞。请把迁移方式切回“实验 · 整页对齐挖洞显中文”后再重新处理。')
            self.experimental_status.setStyleSheet(f'color:{ORANGE}; background:{ORANGE_SOFT}; border:1px solid #F0D4A5; border-radius:8px; padding:6px 8px;')
            self.experimental_status.setToolTip('橙色 = 开关已开，但当前并不会运行实验路线')
            self.mode.setToolTip('实验开关已开，但当前下拉框不是实验项，因此不会运行整页对齐挖洞显中文')
    ...
    def _update_experimental_result_status(self, index: int | None = None):
        if not hasattr(self, "experimental_result_status"):
            return
        if not self.show_experimental.isChecked():
            self.experimental_result_status.setVisible(False)
            return
        ...
        if not (0 <= idx < len(pairs)):
            ...
            self.experimental_result_status.setStyleSheet(f"color:{MUTED};")
            return
        pair = pairs[idx]
        ...
        if not payload:
            ...
            self.experimental_result_status.setStyleSheet(f"color:{ORANGE}; background:{ORANGE_SOFT}; border:1px solid #F0D4A5; border-radius:8px; padding:6px 8px;")
            return
        ...
        if used and accepted and not missing:
            ...
            self.experimental_result_status.setStyleSheet(f"color:{GREEN}; background:{GREEN_SOFT}; border:1px solid #BFE3D5; border-radius:8px; padding:6px 8px;")
            ...
            self.experimental_result_status.setStyleSheet(f"color:{RED}; background:{RED_SOFT}; border:1px solid #E7AEB7; border-radius:8px; padding:6px 8px;")
        else:
...
class ModelPage(QWidget):
    def __init__(self, window: "StudioWindow"):
        ...
        status = Card("模型下载与接入状态", "不会后台静默下载；只有你点击“下载/校验”后才联网。下载成功后自动切成本地离线路径。")
        self.status_labels = {}
        self.model_download_buttons = {}
        downloadable = {"paddle", "lightglue", "loftr", "mangalens", "rtdetr_v2", "sam2"}
        status_rows = [
            ("paddle","PP-OCRv5"), ("lightglue","LightGlue"), ("loftr","LoFTR"),
            ("mangalens","MangaLens"), ("rtdetr_v2","RT-DETR-v2"), ("sam2","SAM 2.1"),
            ("torch_sr","MPS 局部超分"), ("apple_live_text","Apple Live Text"),
            ("apple_shortcut","ExtractText 快捷指令"),
        ]
        for key,name in status_rows:
            line=QHBoxLayout(); line.setSpacing(6)
            name_label=QLabel(name); name_label.setMinimumWidth(88); line.addWidget(name_label)
            line.addStretch(1)
            ...
            if key in downloadable:
                if key == "paddle":
                    dep=QPushButton("安装依赖"); dep.setObjectName("modelDownload"); dep.setMinimumWidth(68); dep.setMaximumWidth(82)
                    dep.clicked.connect(self._install_paddle_dependencies); self.paddle_dependency_button=dep; line.addWidget(dep)
            ...
            status.layout.addLayout(line)

        net=QGridLayout(); net.setHorizontalSpacing(6); net.setVerticalSpacing(5)
        net.addWidget(QLabel("下载源"),0,0)
        self.model_source=QComboBox(); self.model_source.addItem("自动：官方 → 国内镜像","auto"); self.model_source.addItem("仅官方直连","official"); self.model_source.addItem("国内 HF 镜像优先","mirror"); net.addWidget(self.model_source,0,1)
        ...
        self.apply_model_network=QPushButton("应用"); self.test_model_network=QPushButton("网络诊断"); net.addWidget(self.apply_model_network,0,2); net.addWidget(self.test_model_network,1,2)
        settings=QSettings("MangaHDTransfer","MangaHDTransferStudio")
        saved_source=str(settings.value("models/hf_source","auto") or "auto")
        idx=self.model_source.findData(saved_source); self.model_source.setCurrentIndex(max(0,idx))
        self.model_proxy.setText(str(settings.value("models/proxy","") or ""))
        ...
        self.model_download_status=QLabel("支持主动下载、代理/镜像回退和离线导入。PP-OCR 依赖缺失时可直接点击“安装依赖”。")
        ...
        model_actions=QHBoxLayout(); self.open_model_dir=QPushButton("打开模型目录"); self.open_model_dir.clicked.connect(self._open_model_dir); model_actions.addWidget(self.open_model_dir); model_actions.addStretch(1); status.layout.addLayout(model_actions)
    ...
    def _apply_model_network_settings(self, _checked=False, *, show_message: bool = True):
        ...
        apply_download_network_settings(proxy=proxy,hf_source=source)
        settings=QSettings("MangaHDTransfer","MangaHDTransferStudio")
        settings.setValue("models/hf_source",source); settings.setValue("models/proxy",proxy)
        if show_message:
            proxy_msg = proxy if proxy else "系统直连/系统代理"
            self.model_download_status.setText(f"下载网络设置已应用：{self.model_source.currentText()} · {proxy_msg}")
    ...
    def _install_paddle_dependencies(self):
        if self._dependency_worker is not None and self._dependency_worker.isRunning():
            return
        self._apply_model_network_settings(show_message=False)
        reply=QMessageBox.question(self,"安装 PP-OCR 依赖","将使用当前 GUI 的 Python 环境安装 PaddlePaddle 与 PaddleOCR。macOS 会优先使用 Paddle 官方 CPU wheel 源。继续吗？")
        if reply!=QMessageBox.StandardButton.Yes:
            return
        self.model_download_status.setText("正在安装 PP-OCR 依赖…")
        self.model_progress.setVisible(True); self.model_progress.setRange(0,0)
        self.paddle_dependency_button.setEnabled(False)
        worker=DependencyInstallWorker("paddle"); self._dependency_worker=worker
    ...
    def _dependency_install_done(self, result):
        ...
        QMessageBox.information(self,"依赖安装完成",msg)
    ...
    def _dependency_install_failed(self, message: str):
        ...
        QMessageBox.warning(self,"PP-OCR 依赖安装失败",short)

    def _dependency_install_finished(self):
        self._dependency_worker=None
        if hasattr(self,"paddle_dependency_button"):
            self.paddle_dependency_button.setEnabled(True)
    ...
    def _import_model(self, key: str):
        if key in {"paddle","rtdetr_v2"}:
            source=QFileDialog.getExistingDirectory(self,"选择离线模型目录")
        else:
            filters={"lightglue":"PyTorch 权重 (*.pth *.pt);;所有文件 (*)","loftr":"LoFTR checkpoint (*.ckpt *.pth);;所有文件 (*)","mangalens":"YOLO 权重 (*.pt);;所有文件 (*)","sam2":"SAM2 checkpoint (*.pt *.pth);;所有文件 (*)"}
    ...
    def _download_model(self, key: str):
        ...
        sizes={"paddle":"首次预热可能下载数百 MB","lightglue":"约 45 MB","loftr":"约 45 MB","mangalens":"约 12 MB","rtdetr_v2":"约 172 MB + 配置","sam2":"约 156 MB"}
        label={"paddle":"PP-OCRv5 中/日文","lightglue":"LightGlue SIFT","loftr":"LoFTR outdoor","mangalens":"MangaLens","rtdetr_v2":"RT-DETR-v2","sam2":"SAM 2.1 Hiera Tiny"}.get(key,key)
    ...
    def _model_download_failed(self, message: str):
        ...
        QMessageBox.warning(self,"模型下载失败",short)
    ...
    def _apply_probe_statuses(self, statuses):
        ...
        self.device_status.setText(f"计划设备：{selected} · {mps.detail}")
        ...
        for key,st in statuses.items():
            if key in self.status_labels:
                ...
                if key in self.model_download_buttons and self._model_download_worker is None:
                    if key == "paddle":
                        self.model_download_buttons[key].setText("重新预热" if st.ready else "下载/预热")
                    else:
...
class WorkbenchPage(QWidget):
    def __init__(self, window: "StudioWindow"):
        ...
        preview=Card()
        preview.layout.setContentsMargins(10,8,10,8); preview.layout.setSpacing(5)
        page_nav=QHBoxLayout(); page_nav.setSpacing(6)
        compact_title=QLabel("替换工作台"); compact_title.setObjectName("sectionTitle")
        page_nav.addWidget(compact_title); page_nav.addStretch(1)
        ...
        page_nav.addWidget(self.prev_page); page_nav.addWidget(self.page_counter); page_nav.addWidget(self.next_page); page_nav.addSpacing(4); page_nav.addWidget(self.inspector_toggle)
        ...
        for i,(label,key) in enumerate([("日文原图","target"),("旧中文版","source"),("最终结果","result"),("复核标注","review"),("迁移蒙版","mask"),("清除蒙版","clear_mask"),("中文迁移层","chinese_layer"),("只清日文","removed"),("TARGET擦除预览","target_erase")]):
        ...
        sl=QVBoxLayout(host); sl.setContentsMargins(0,0,3,0); sl.setSpacing(8); side.setWidget(host)
        ...
        sr1=QHBoxLayout(); self.edit_clear_mask=QPushButton("编辑自动清除蒙版…"); self.remove_text_only=QPushButton("预览自动去字"); sr1.addWidget(self.edit_clear_mask,1); sr1.addWidget(self.remove_text_only,1); stages.layout.addLayout(sr1)
        ...
        sr3=QHBoxLayout(); self.target_erase_mode=QComboBox(); self.target_erase_mode.addItem("智能恢复 TARGET", "auto"); self.target_erase_mode.addItem("纯白涂抹", "pure_white"); self.target_erase_mode.setToolTip("智能恢复：白底用 TARGET 纸色、复杂背景用 inpaint；纯白涂抹：把有效笔刷区域直接写成纯白，中文保护区仍保持不变。")
        self.target_layer_erase=QPushButton("仅擦 TARGET 日文层…"); self.target_layer_erase.setObjectName("softPrimary"); self.reset_target_layer_erase=QPushButton("清空 TARGET 擦除"); sr3.addWidget(self.target_erase_mode); sr3.addWidget(self.target_layer_erase,1); sr3.addWidget(self.reset_target_layer_erase,1); stages.layout.addLayout(sr3)
        self.target_layer_erase.setToolTip("最终收尾画笔：可刷日文残字、黑点、短线和符号。只重建 TARGET 母版层，中文迁移/重排/人工补漏图层自动保护。")
        stage_note=QLabel("“编辑自动清除蒙版”用于修正自动清字管线；“仅擦 TARGET 日文层”才是默认最终收尾工具：直接在已有中文的结果上刷残留日文、黑点、短线和符号，中文图层硬保护，即使画笔压到中文也不会擦掉，中文像素也保持不变。")
        ...
        # Compact inspector density: keep the page preview visually dominant
        # while still allowing all controls to wrap inside the sidebar.
        for card in host.findChildren(Card):
            card.layout.setContentsMargins(10,8,10,8)
            card.layout.setSpacing(7)
        side.setMinimumWidth(220); side.setMaximumWidth(280); split.addWidget(side); self.split=split; self.preview_card=preview
        split.setSizes([1120,248]); split.setStretchFactor(0,1); split.setStretchFactor(1,0); split.setChildrenCollapsible(False)
        self._compact_layout=None
        QTimer.singleShot(0, self._update_responsive_workbench)
        self.prev_page.clicked.connect(lambda: self._move_page(-1)); self.next_page.clicked.connect(lambda: self._move_page(1)); self.inspector_toggle.toggled.connect(self._toggle_inspector)
        ...
        self.bubble_backend.currentIndexChanged.connect(self._sync); self.detector_size.valueChanged.connect(self._sync); self.clear_dilate.valueChanged.connect(self._sync); self.inpaint_backend.currentIndexChanged.connect(self._sync); self.expand_direct_range.toggled.connect(self._sync)
    ...
    def _update_responsive_workbench(self):
        ...
        if compact:
            ...
            self.side.setMinimumWidth(220); self.side.setMaximumWidth(280)
            w=max(900,self.width())
            side_w=min(265,max(228,int(w*0.16)))
            self.split.setSizes([max(650,w-side_w),side_w])
    ...
    def _edit_clear_mask(self):
        ...
        self.window.statusBar().showMessage("已保存自动清除蒙版修订；可先点“预览自动去字”检查。最终零星残留建议使用“仅擦 TARGET 日文层”。",5000); self.refresh()
    ...
    def _edit_target_layer_erase(self):
        ...
        def _target_erase_live_preview(mask_value):
            ...
            if selected_fill_mode == "pure_white":
                cleaned=target.copy(); cleaned[expanded>0]=255
            else:
                cleaned=cv2.inpaint(target,expanded,3.0,cv2.INPAINT_TELEA)
            out=display.copy(); out[expanded>0]=cleaned[expanded>0]; return out
    ...
    def _commit_manual_effect_dialog_result(self, page_dir: Path, row: dict[str, Any], reveal, reveal_patch, preset_candidate: dict[str, Any] | None = None) -> Path:
        ...
        if remembered_mode:
            QSettings("MangaHDTransfer", "MangaHDTransferStudio").setValue("review/last_manual_effect_mode", remembered_mode)
        pair=self.window.current_pair(); page_id=page_id_for_pair(pair) if pair is not None else ""; proj=self.window.state.projects_by_page.get(page_id)
        if proj is not None:
    ...
    def _add_manual_effect_region(self, preset_candidate: dict[str, Any] | None = None):
        ...
        if preset_candidate:
            ...
            if not remembered:
                remembered = str(QSettings("MangaHDTransfer", "MangaHDTransferStudio").value("review/last_manual_effect_mode", "") or "")
    ...
    def refresh(self):
        ...
        if current_path:
            if not page_mark.should_process:
                ...
                self.view_status.setText((f"已恢复已有结果 · {origin} · 可继续人工补漏" if origin else "已同步到当前页"))
...
class ExportPage(QWidget):
    def __init__(self, window: "StudioWindow"):
        ...
        self.cleanup=QPushButton("清理 pages 冗余诊断文件…")
...
class StudioWindow(QMainWindow):
    ...
    def _vertical_nav_label(label: str) -> str:
        """Render CJK workflow labels top-to-bottom without rotating glyphs."""
        return "\n".join(str(label))
    ...
    def __init__(self):
        ...
        self.workflow_rail.setFixedWidth(94)
        rail = QVBoxLayout(self.workflow_rail); rail.setContentsMargins(11,12,11,10); rail.setSpacing(7)

        brand = QLabel("M\nH\nD"); brand.setObjectName("railBrand")
        brand.setAlignment(Qt.AlignmentFlag.AlignCenter)
        brand.setToolTip(APP_NAME)
        rail.addWidget(brand)
        caption = QLabel("工作流"); caption.setObjectName("railCaption")
        ...
        version.setToolTip("Manga HD Transfer Studio")
        ...
        top = QFrame(); top.setObjectName("topbar"); top.setFixedHeight(50)
        tl = QHBoxLayout(top); tl.setContentsMargins(14,5,14,5); tl.setSpacing(9)
        self.page_heading = QLabel("页面管理"); self.page_heading.setObjectName("appTitle")
        self.workflow_step = QLabel("WORKFLOW 01 · PAGE MANAGER"); self.workflow_step.setObjectName("workflowStep")
        sub = QLabel("旧版中文 → 高清日文 · Direct / 精准蒙版 · 页面级 QA · 本地处理")
        sub.setObjectName("appSubtitle")
        sub.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred)
        tl.addWidget(self.page_heading)
        tl.addWidget(self.workflow_step)
        tl.addSpacing(4)
        tl.addWidget(sub, 1)
        ...
        badge = QLabel("MAC · MPS"); badge.setObjectName("badge"); tl.addWidget(badge)
        ...
        for page in [self.project, self.models, self.workbench, self.export]:
    ...
    def show_page(self, index: int):
        ...
        if 0 <= index < len(self.NAV_SECTIONS):
            label, english = self.NAV_SECTIONS[index]
            self.page_heading.setText(label)
            self.workflow_step.setText(f"WORKFLOW {index + 1:02d} · {english}")
    ...
    def restore_existing_results(self):
        ...
        try:
            ...
            self.statusBar().showMessage(f"已恢复 {len(session.pages)} 页已有结果{warning}。可直接进入替换工作台继续人工补漏。", 8000)
            self.refresh_all()
            self.show_page(1)
        except Exception as exc:
