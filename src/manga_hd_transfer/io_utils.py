...
def write_image(path: str | Path, image: np.ndarray, params: list[int] | None = None) -> None:
    """Encode then atomically replace one image artifact.

    Page processing writes many masks/layers that may be read immediately by the
    GUI or review pipeline.  Writing encoded bytes directly to the destination
    exposes a truncated file if the process is interrupted mid-write.  The image
    is already fully encoded in memory, so a sibling-temp + ``os.replace`` adds
    negligible CPU cost while preventing half-written PNG/JPEG artifacts.
    """
    p = Path(path)
    p.parent.mkdir(parents=True, exist_ok=True)
    ext = p.suffix or ".png"
    encode_params = params
    if encode_params is None and ext.casefold() == ".png":
        # OpenCV's implicit PNG setting is unnecessarily large for manga pages.
        # Level 3 is still fast, lossless and typically trims another 15-25%.
        encode_params = [cv2.IMWRITE_PNG_COMPRESSION, 3]
    ok, encoded = cv2.imencode(ext, image, encode_params or [])
    if not ok:
        raise ValueError(f"Could not encode image as {ext}: {p}")
    tmp_name = None
    try:
        try:
            desired_mode = stat.S_IMODE(p.stat().st_mode)
        except OSError:
            desired_mode = 0o644
        with tempfile.NamedTemporaryFile(
            mode="wb", dir=str(p.parent), prefix=f".{p.name}.", suffix=".tmp", delete=False,
        ) as fh:
            tmp_name = fh.name
