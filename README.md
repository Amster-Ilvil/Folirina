<p align="center">
  <img src="assets/folirina_banner.svg" alt="Folirina — Manga Translation Transfer Studio" width="100%">
</p>
...
> 本地处理 · 页面配准 · Direct / Mask / Reveal · OCR 与模型中心 · 人工补漏 · 可恢复编辑 · 批量处理 · Windows / macOS / Linux
...
- **多种替换模式**：提供 Direct Patch、Mask Replace、Aligned Overlay Reveal、Transparent Bubble Reveal、Hybrid 与 Reletter 等处理路径。
...
- **核心功能保持不变**：本次多平台适配集中在打包与发布层，没有重写已稳定的 Direct、Mask、Reveal 与 Reletter 核心流程。

## 使用流程

1. 导入来源漫画与目标 / 翻译页面。
2. 建立或检查页面配对与结构对齐。
3. 根据页面类型选择自动、Direct、Mask、Reveal、Hybrid 或 Reletter 路径。
4. 运行页面或整册处理。
...
| 模式 | 用途 |
|---|---|
| `auto` | 根据页面证据自动选择合适处理路径 |
| `direct_patch` | 对高置信、结构一致的文字 / 气泡区域进行直接迁移 |
| `mask_replace` | 通过安全蒙版限制替换区域，尽量保留边框与背景 |
| `aligned_overlay_reveal` | 对齐上下层页面，通过局部显露完成文字层迁移 |
| `transparent_bubble_reveal` | 面向气泡 / 文本框的透明显露式处理 |
| `hybrid` | 综合多种证据，在不同处理策略间选择或回退 |
| `reletter` | 对文字区域重新排字 / 重建文字表现 |
