...
# Keep this launcher ASCII-only for Windows PowerShell 5.1 compatibility.
...
function Test-SupportedPython([string]$Executable) {
    ...
    & $Executable -c "import sys;raise SystemExit(0 if sys.version_info.major==3 and 11<=sys.version_info.minor<=13 else 8)" 2>$null
    return ($LASTEXITCODE -eq 0)
}

function Find-SystemPython {
    if ($env:MHD_TRANSFER_PYTHON -and (Test-SupportedPython $env:MHD_TRANSFER_PYTHON)) {
        return $env:MHD_TRANSFER_PYTHON
    }
...
function Invoke-Download([string[]]$Urls, [string]$Destination) {
    ...
    foreach ($url in $Urls) {
        ...
        Write-Stage 'Downloading the verified Python runtime.'
        try {
            Import-Module BitsTransfer -ErrorAction Stop
            Start-BitsTransfer -Source $url -Destination $part -DisplayName 'Manga HD Transfer deployment' -Description 'First deployment runtime' -ErrorAction Stop
    ...
    throw 'All runtime download sources failed. Check the network or proxy settings.'
}
...
function Install-PortablePython {
    ...
    if (-not $line) { throw 'The checksum file does not contain the required Python archive.' }
    ...
    if ($expected -ne $actual) {
        ...
        throw 'Standalone Python SHA-256 verification failed. Downloaded files were removed.'
    }
    ...
    if (-not $tar) { throw 'tar.exe is unavailable; standalone Python cannot be extracted.' }
...
try {
    Write-Stage 'Starting Windows deployment check.'
    $basePython = Find-SystemPython
    if (-not $basePython) {
        Write-Stage 'No compatible Python found. Preparing a verified portable runtime.'
        $basePython = Install-PortablePython
    } else {
        Write-Stage 'Compatible Python found.'
    }

    if (-not (Test-SupportedPython $PythonExe)) {
        Remove-Item -LiteralPath $AppVenv -Recurse -Force -ErrorAction SilentlyContinue
        Write-Stage 'Creating the application virtual environment.'
    ...
    $arguments = @((Join-Path $ProjectDir 'bootstrap.py'), '--install-main-deps')
    if (-not $PrepareOnly) { $arguments += '--launch' }

    Write-Stage 'Preparing main application dependencies only. OCR/ML resources remain user-confirmed and on-demand.'
    ...
    Write-Stage 'Launcher completed successfully.'
    exit 0
}
catch {
    ...
    if ('System.Windows.MessageBox' -as [type]) {
        [System.Windows.MessageBox]::Show($message, 'Manga HD Transfer Studio startup failed', 'OK', 'Error') | Out-Null
    }
    exit 1
}
